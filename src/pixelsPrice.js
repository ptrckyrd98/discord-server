...
function getPrices() {
  ...
  axios
    .get(
      `https://api.coingecko.com/api/v3/coins/markets?vs_currency=${process.env.PREFERRED_CURRENCY}&ids=${process.env.PIXELS_ID}`
    )
    .then((res) => {
      ...
      if (
        ...
        client.guilds.cache
          ...
          .me.setNickname(
            `PIXELS ${process.env.CURRENCY_SYMBOL}${currentPrice
              .toLocaleString()
...
// Login to Discord
client.login(process.env.PIXELS_BOT_TOKEN);
