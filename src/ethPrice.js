...
function getPrices() {
	...
	// API for price data.
	axios.get(`https://api.coingecko.com/api/v3/coins/markets?vs_currency=${process.env.PREFERRED_CURRENCY}&ids=${process.env.ETH_ID}`).then(res => {
		...
		if(res.data && res.data[0].current_price && res.data[0].price_change_percentage_24h) {
			...
			client.guilds.cache.find(guild => guild.id === process.env.SERVER_ID).me.setNickname(`ETH ${process.env.CURRENCY_SYMBOL}${(currentPrice).toLocaleString().replace(/,/g,process.env.THOUSAND_SEPARATOR)}`)
			console.log('Updated price to', currentPrice)
		}
		else
			console.log('Could not load player count data for', process.env.ETH_ID)
