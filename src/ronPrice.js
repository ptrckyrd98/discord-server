...
function getPrices() {
  ...
  axios
    .get(
      `https://api.coingecko.com/api/v3/coins/markets?vs_currency=${process.env.PREFERRED_CURRENCY}&ids=${process.env.RON_ID}`
    )
    .then((res) => {
      ...
      if (
        ...
        client.guilds.cache
          ...
          .me.setNickname(
            `RON ${process.env.CURRENCY_SYMBOL}${currentPrice
              .toLocaleString()
              .replace(/,/g, process.env.THOUSAND_SEPARATOR)}`
          );
        console.log("Updated price to", currentPrice);
      } else
        console.log("Could not load player count data for", process.env.RON_ID);
    })
...
// Login to Discord
client.login(process.env.RON_BOT_TOKEN);
